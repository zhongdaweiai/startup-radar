...
const COMPANY_ACTION_PATTERN =
  /^(.{2,80}?)\s+(?:raises?|raised|lands?|secures?|gets|nabs?|launch(?:es|ed)?|unveils?|introduces?|rolls?\s+out|debuts?|acquires?|buys?|snaps?\s+up|merges?|partners?|teams?\s+up|sues?|cuts?|lays?\s+off|files?|creates?|created|builds?|built|adds?|announces?|announced)\b/i;
...
function isCompanyCandidate(value) {
  ...
  if (COMPANY_STOP_WORDS.has(candidate)) {
    return false;
  }
...
function extractCompanySignals(title) {
  const signals = [];
  ...
  for (const knownCompany of KNOWN_COMPANIES) {
    ...
    if (new RegExp(`\\b${escaped}\\b`, "i").test(normalizedTitle)) {
...
export function extractStorySignals({ title, categories = [] }) {
  const normalizedTitle = clean(title);
  const haystack = [normalizedTitle, ...categories.map(clean)].filter(Boolean).join(" ");
  ...
  return uniqueSignals([
    ...signalsFromRules("event", EVENT_RULES, haystack),
    ...extractCompanySignals(normalizedTitle),
