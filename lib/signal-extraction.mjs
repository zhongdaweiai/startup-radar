...
const COMPANY_LIKE_TERMS =
  /\b(?:AI|Labs?|Robotics|Systems|Technologies|Software|Analytics|Health|Capital|Ventures|Cloud|Data|DB|API|Bank|Security|Networks?|Platform|Compute)\b/;
...
function isCompanyCandidate(value) {
  ...
  if (COMPANY_STOP_WORDS.has(candidate) || NON_COMPANY_PHRASES.has(candidate)) {
    return false;
  }
...
export function extractStorySignals({ title, categories = [] }) {
  const normalizedTitle = clean(title);
  const categoryHaystack = categories.slice(0, 1).map(clean).filter(Boolean).join(" ");
  const haystack = [normalizedTitle, categoryHaystack].filter(Boolean).join(" ");
